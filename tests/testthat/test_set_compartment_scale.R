...
test_that("set_compartment_scale infers compartment 2 for ADVAN 2, 4, 12", {
  model <- list(code = "S1 = V1\nother code")
  
  for (advan in c(2, 4, 12)) {
    mockery::stub(set_compartment_scale, "get_advan", advan)
    ...
    expect_message(
      ...
      "No scaling specified for compartment 2"
    )
  }
})
...
test_that("set_compartment_scale infers compartment 1 for ADVAN 1, 3, 11", {
  ...
  for (advan in c(1, 3, 11)) {
    ...
    expect_message(
      result <- set_compartment_scale(model, expression = list(variable = "V", scale = 1000)),
      "No scaling specified for compartment 1"
    )
  }
})

test_that("set_compartment_scale warns for non-standard ADVAN", {
  model <- list(code = "S1 = V1\nother code")
  
  mockery::stub(set_compartment_scale, "get_advan", 99) # non-standard ADVAN
  mockery::stub(set_compartment_scale, "get_compartment_scale", NULL)
  mockery::stub(set_compartment_scale, "pharmr::read_model_from_string", 
    function(code) list(code = code))
  mockery::stub(set_compartment_scale, "scale_initial_estimates_pk", 
  ...
  expect_warning(
    result <- set_compartment_scale(model, expression = list(variable = "V", scale = 1000)),
    "No `compartment` specified to scale, assuming compartment 1"
  )
})
